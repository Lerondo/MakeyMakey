...
public class ExampleClass : MonoBehaviour {
	...
	void OnGUI()
	{
		if (GUI.Button (new Rect(Screen.width * guiPlacementX1, Screen.height * guiPlacementY1, Screen.width * .075f, Screen.height * .10f), "",Random1)){
			//score = GameObject.FindGameObjectWithTag("Arrows").GetComponent<ArrowMovement>();
			//score.scoreValue = 0;
		...
		if (lastframe && thisframe) {
			...
			if(bassframes > 2.5)
			{
				...
				case 2:
					...
					if(cases >= 2)
					{
						//print ("Up Arrow Spawn");
						Instantiate (ArrowUp, UpSpawn.position, UpSpawn.rotation);
						cases = 0;
					}
					break;
				case 0:
